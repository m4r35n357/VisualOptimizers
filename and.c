/*
 * Artificial Neural Network simulating AND gate function
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void cost (int n, point *p, const model *m)  { (void)n; (void)m;
    ...
    p->f += SQR(0.0L - nn(0.0L, p->x[0], 1.0L, p->x[1], p->x[2]));
    p->f += SQR(0.0L - nn(1.0L, p->x[0], 0.0L, p->x[1], p->x[2]));
    p->f += SQR(1.0L - nn(1.0L, p->x[0], 1.0L, p->x[1], p->x[2]));

    p->f += SQR(0.0L - nn(0.2L, p->x[0], 0.2L, p->x[1], p->x[2]));
    p->f += SQR(0.0L - nn(0.2L, p->x[0], 0.8L, p->x[1], p->x[2]));
    p->f += SQR(0.0L - nn(0.8L, p->x[0], 0.2L, p->x[1], p->x[2]));
